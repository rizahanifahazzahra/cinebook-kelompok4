...
function saveBooking() {
  ...
  const newBooking = {
    ...
    price: totalPrice,
    method: method,
    status: 'pending' // Kursi ter-reservasi, menunggu pembayaran dikonfirmasi
  };
  ...
  alert('Kursi berhasil dipesan dan direservasi.\nStatus: Menunggu Pembayaran.\nKlik tombol "Bayar" pada tabel di bawah untuk menyelesaikan transaksi.');
  resetForm();
  renderTable();
}

// Menyelesaikan pembayaran untuk pesanan yang masih berstatus pending
...
function renderTable() {
  ...
  if (bookings.length === 0) {
    tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; color: #888; padding: 20px;">Belum ada riwayat transaksi.</td></tr>`;
    return;
  }
  ...
  bookings.forEach(b => {
    ...
    // Data lama (sebelum fitur status ditambahkan) dianggap sudah lunas
    const status = b.status || 'paid';
    const statusBadge = status === 'paid'
      ? '<span class="badge-paid">LUNAS</span>'
      : '<span class="badge-pending">Menunggu Bayar</span>';
    const payButton = status === 'pending'
      ? `<button class="btn-action btn-pay" onclick="payBooking('${b.id}')">Bayar</button>`
      : '';
...
function openEditModal(id) {
  ...
  document.getElementById('editInfoStatus').innerText = isPaid ? 'LUNAS' : 'Menunggu Pembayaran';

  // Metode pembayaran hanya boleh diubah selama pesanan belum lunas
  document.getElementById('editPaymentMethod').disabled = isPaid;
  document.getElementById('editLockNote').style.display = isPaid ? 'inline' : 'none';
